...
export const create = async (
  ...
  const { data } = await baseApi.post(BASE_URL + "sessions/", params);
...
> => {
  const { data } = await baseApi.get(BASE_URL + "sessions/active/");
...
export const deleteSingle = async (
  ...
  const { data } = await baseApi.delete(BASE_URL + "sessions/", {
    data: params,
  });
