...
export const login = async (loginData: UserLoginSubmit): Promise<ResponseSingle<LoginResponse>> => {
    const resp = await baseApi.post<ResponseSingle<LoginResponse>>('auth/login', { ...loginData });
    return resp.data;
}
